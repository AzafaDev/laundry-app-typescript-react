...
import { useChangeStaffPasswordMutation } from "../../hooks/staffAuth/useChangeStaffPasswordMutation";
...
export function StaffChangePasswordSection() {
  ...
  const mutation = useChangeStaffPasswordMutation();
  ...
  return (
    <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
      ...
      <FormField label="Kata sandi saat ini" htmlFor="current_password" error={errors.current_password?.message}>
        <PasswordInput id="current_password" autoComplete="current-password" autoFocus {...register("current_password")} />
      </FormField>
